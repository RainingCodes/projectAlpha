...
def generate_launch_description():
    ...
    scenario_desc = os.path.join(pkg_stonefish_ros2, 'scenarios', 'fls_test.scn')
    ...
    # Goal position arguments - straight line path
    goal_x_arg = DeclareLaunchArgument(
        'goal_x',
        default_value='20.0',
        description='Goal X coordinate (20m straight ahead)'
    )

    goal_y_arg = DeclareLaunchArgument(
        'goal_y',
        default_value='0.0',
        description='Goal Y coordinate (straight line)'
    )
    ...
    fls_viewer_node = Node(
        ...
        parameters=[
            {'namespace': 'GIRONA500'},
            {'window_name': 'FLS - Forward Looking Sonar'},
            {'horizontal_fov': 90.0},
            {'range_min': 1.0},
            {'range_max': 10.0},
            {'display_size': 400},
