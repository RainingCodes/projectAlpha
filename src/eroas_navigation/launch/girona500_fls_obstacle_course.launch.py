...
def generate_launch_description():
    ...
    scenario_desc = os.path.join(pkg_stonefish_ros2, 'scenarios', 'fls_obstacle_course.scn')
    ...
    # Waypoint arguments - canyon passage path
    # Path: Start(0,0) -> WP1(18,0) -> WP2(23,3) -> WP3(28,5) -> Goal(42,5)
    # Canyon 1 (x=3~18, y=0 center) -> Turn -> Canyon 2 (x=28~42, y=5 center)
    waypoint_x_arg = DeclareLaunchArgument(
        'waypoint_x',
        default_value='[18.0, 23.0, 28.0, 42.0]',
        description='X coordinates (North in NED)'
    )

    waypoint_y_arg = DeclareLaunchArgument(
        'waypoint_y',
        default_value='[0.0, 3.0, 5.0, 5.0]',
        description='Y coordinates (East in NED)'
    )

    waypoint_z_arg = DeclareLaunchArgument(
        'waypoint_z',
        default_value='[3.0, 3.0, 3.0, 3.0]',
        description='Z coordinates (Down in NED)'
    )

    waypoint_tolerance_arg = DeclareLaunchArgument(
        'waypoint_tolerance',
        default_value='1.5',
        description='Waypoint arrival tolerance (meters)'
    )
    ...
    # FLS Viewer node (polar fan display)
    fls_viewer_node = Node(
        ...
        parameters=[
            {'namespace': 'GIRONA500'},
            {'window_name': 'FLS - Obstacle Course'},
            {'horizontal_fov': 90.0},
            {'range_min': 1.0},
            {'range_max': 10.0},
            {'display_size': 400},
        ...
        remappings=[
            ('/GIRONA500/fls', '/GIRONA500/fls/image')
        ],
        output='screen',
    )
    ...
    fls_recorder_node = Node(
        package='eroas_navigation',
        executable='fls_recorder',
        name='fls_recorder',
        parameters=[
            {'namespace': 'GIRONA500'},
            {'output_dir': '/tmp/fls_obstacle_course'},
            {'record_enabled': True}
        ],
        remappings=[
            ('/GIRONA500/fls', '/GIRONA500/fls/image')
